...
`POST /single_search/company` is the same action under an older path and behaves identically. Both
are supported, so nothing breaks if you already call the old one.

<Note>
  This is the endpoint the Lead Finder points you at when a search turns out to be a single company
  lookup. See [Errors](/api-reference/errors).
</Note>
