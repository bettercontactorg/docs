...
| `404` | Unknown Lead Finder `request_id`, or no company matched on [Enrich a company](/api-reference/enrich_profile/company). | Check the input, do not retry. |
...
| `You're looking to enrich a company, please use dedicated endpoint: /api/v2/single_search/company` | The search targets a single company domain with `limit: 1` and no other filter. That is a company lookup, not a search: use [Enrich a company](/api-reference/enrich_profile/company) instead. |
| `Lead finder was not accepted.` | The request failed validation, most often `limit`, `offset` or `max_leads` out of bounds. |
| `company_domain or company_linkedin_url is required.` | [Enrich a company](/api-reference/enrich_profile/company) was called without any identifier. |
