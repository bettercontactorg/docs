...
<Warning>
  On [Enrich a lead profile](/api-reference/enrich_profile/lead), an unexpected internal failure
  comes back as `200 OK` with `success: false`. Read `success` on every synchronous endpoint rather
